...
function NgoDetails() {
  ...
  const [title, setTitle] = useState("Orange Tree Foundation");
  return (
    <>
      <ScrollView>
        <AppText style={styles.heading}>{ngo[0].ngo.name}</AppText>
        <NgoInfo />
        ...
        <View style={styles.ngoDetailsCont}>
          ...
          <AppText style={styles.subtext}>{ngo[0].about_us}</AppText>

          <Divider orientation="center">
            <AppText style={styles.subheadings}>Our Services</AppText>
          </Divider>
          <AppText style={styles.subtext}>{ngo[0].services}</AppText>
        </View>
      </ScrollView>
    </>
  );
